...
long long make(long long node,long long left,long long right){
    ...
    long long leftSum=make(node*2,left,mid);
    long long rightSum=make(node*2+1,mid+1,right);
    return segment[node]=leftSum+rightSum;
}
...
long long change(long long node,long long nodeLeft,long long nodeRight,long long idx,long long newV){
    ...
    long long mid=(nodeLeft+nodeRight)/2;
    return segment[node]=(change(node*2,nodeLeft,mid,idx,newV)+change(node*2+1,mid+1,nodeRight,idx,newV));
}
